use nave_apply::{
    AdapterState, BranchEnvelope, BranchRepoRequest, BranchState, CommitEnvelope,
    CommitRepoRequest, PROTOCOL_VERSION, PushEnvelope, PushRepoRequest, ResetEnvelope,
    ResetRepoRequest, ValidationError, validate_bound_path, validate_envelope_repos,
    validate_hex_sha, validate_ref_name,
};
...
fn empty_repos_is_rejected() {
    assert!(matches!(
        validate_envelope_repos(PROTOCOL_VERSION, &[]),
        Err(ValidationError::EmptyRepos)
    ));
}
...
fn duplicate_repo_is_rejected() {
    ...
    assert!(matches!(
        validate_envelope_repos(PROTOCOL_VERSION, &repos),
        Err(ValidationError::DuplicateRepo(_))
    ));
}
...
fn non_owner_name_identity_is_rejected() {
    ...
    assert!(matches!(
        validate_envelope_repos(PROTOCOL_VERSION, &repos),
        Err(ValidationError::InvalidRepoIdentity(_))
    ));
}
...
fn branch_state_serializes_kebab_case() {
    assert_eq!(
        serde_json::to_string(&BranchState::StaleBase).unwrap(),
        "\"stale-base\""
    );
}
...
fn adapter_state_serializes_snake_case() {
    assert_eq!(
        serde_json::to_string(&AdapterState::Error).unwrap(),
        "\"error\""
    );
}
...
fn branch_envelope_serializes_repos_sorted_regardless_of_construction_order() {
    let e1 = BranchEnvelope {
        protocol_version: 1,
        apply_ref: "pulse/apply/p1".into(),
        repos: vec![branch_req("z/z"), branch_req("a/a")],
    };
    let e2 = BranchEnvelope {
        protocol_version: 1,
        apply_ref: "pulse/apply/p1".into(),
        repos: vec![branch_req("a/a"), branch_req("z/z")],
    };
    assert_eq!(
...
fn reset_request_carries_expected_pushed_sha_when_present() {
    let raw = format!(
        r#"{{"protocol_version":1,"repos":[{{"repo":"a/b","expected_pushed_sha":"{}"}}]}}"#,
        "a".repeat(40)
    );
    let env: ResetEnvelope = serde_json::from_str(&raw).unwrap();
...
fn result_envelopes_carry_top_level_reason_and_it_is_omitted_when_none() {
    let ok = nave_apply::BranchResult {
        protocol_version: 1,
    ...
    let err = nave_apply::BranchResult {
        protocol_version: 1,
        adapter_state: AdapterState::Error,
